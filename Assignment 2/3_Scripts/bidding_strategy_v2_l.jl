...
# same for 16
idx_match_16 = zeros(Int64, length(dt_16))
for (i, date) in enumerate(dt_16)
	idxs_16 = findall(x -> x == date, df_forecast.dato)
	for (j, idx) in enumerate(idxs_16)
		if (
			(hour(df_forecast.dati[idx]) == 11) ||
			(
			hour(df_forecast.dati[idx]) == 12 &&
			...
			if idx_match_16[i] == 0
				idx_match_16[i] = idx
			elseif df_forecast.hors[idx] < df_forecast.hors[idxs_16[j-1]]
				idx_match_16[i] = idx
			end
		end
	end
...
for i in 1:length(dt_17)
	if idx_match[i] != 0
		...
		revenue_optimal[i] = production/1000 * λ_s
	end
end
...
for i in 1:length(dt_17)
	if idx_match[i] != 0
		...
		revenue_dayahead_base[i] = forecast/1000 * λ_s
		if production > forecast
			revenue_balancing_base[i] = (production - forecast)/1000 * λ_down
		elseif production < forecast
			revenue_balancing_base[i] = -(forecast - production)/1000 * λ_up
		end
	end
end
...
for i in 1:length(dt_17)
	if idx_match[i] != 0
		...
		λ_down = df_market17.Down_10[i]
		λ_up = df_market17.Up_10[i]
		revenue_dayahead_det[i] = forecast/1000 * λ_s
		if production > forecast
			revenue_balancing_det[i] = (production - forecast)/1000 * λ_down
		elseif production < forecast
			revenue_balancing_det[i] = -(forecast - production)/1000 * λ_up
		end
	end
end
...
for i in 1:length(dt_17)
	if idx_match[i] != 0
		...
		λ_down = df_market17.Down_10[i]
		λ_up = df_market17.Up_10[i]
		revenue_dayahead_det2[i] = forecast/1000 * λ_s
		if production > forecast
			revenue_balancing_det2[i] = (production - forecast)/1000 * λ_down
		elseif production < forecast
			revenue_balancing_det2[i] = -(forecast - production)/1000 * λ_up
		end
	end
end
...
for i in 1:length(dt_17)
	if idx_match[i]!=0
		if opt_q[i] <= 1 && opt_q[i] > 0.9
			offer[i] = df_forecast.q95[idx_match[i]]
		elseif opt_q[i] <= 0.9 && opt_q[i] > 0.85
			offer[i] = df_forecast.q90[idx_match[i]]
		elseif opt_q[i] <= 0.85 && opt_q[i] > 0.8
			offer[i] = df_forecast.q85[idx_match[i]]
		elseif opt_q[i] <= 0.8 && opt_q[i] > 0.75
			offer[i] = df_forecast.q80[idx_match[i]]
		elseif opt_q[i] <= 0.75 && opt_q[i] > 0.7
			offer[i] = df_forecast.q75[idx_match[i]]
		elseif opt_q[i] <= 0.7 && opt_q[i] > 0.65
			offer[i] = df_forecast.q70[idx_match[i]]
		elseif opt_q[i] <= 0.65 && opt_q[i] > 0.6
			offer[i] = df_forecast.q65[idx_match[i]]
		elseif opt_q[i] <= 0.6 && opt_q[i] > 0.55
			offer[i] = df_forecast.q60[idx_match[i]]
		elseif opt_q[i] <= 0.55 && opt_q[i] > 0.5
			offer[i] = df_forecast.q55[idx_match[i]]
		elseif opt_q[i] <= 0.5 && opt_q[i] > 0.45
			offer[i] = df_forecast.q50[idx_match[i]]
		elseif opt_q[i] <= 0.45 && opt_q[i] > 0.4
			offer[i] = df_forecast.q45[idx_match[i]]
		elseif opt_q[i] <= 0.4 && opt_q[i] > 0.35
			offer[i] = df_forecast.q40[idx_match[i]]
		elseif opt_q[i] <= 0.35 && opt_q[i] > 0.3
			offer[i] = df_forecast.q35[idx_match[i]]
		elseif opt_q[i] <= 0.3 && opt_q[i] > 0.25
			offer[i] = df_forecast.q30[idx_match[i]]
		elseif opt_q[i] <= 0.25 && opt_q[i] > 0.2
			offer[i] = df_forecast.q25[idx_match[i]]
		elseif opt_q[i] <= 0.2 && opt_q[i] > 0.15
			offer[i] = df_forecast.q20[idx_match[i]]
		elseif opt_q[i] <= 0.15 && opt_q[i] > 0.1
			offer[i] = df_forecast.q15[idx_match[i]]
		elseif opt_q[i] <= 0.1 && opt_q[i] > 0.05
			offer[i] = df_forecast.q10[idx_match[i]]
		elseif opt_q[i] <= 0.05 || opt_q[i] == NaN
			offer[i] = df_forecast.q5[idx_match[i]]
		end
	end
end
...
revenue_balancing_prob = zeros(length(offer))
revenue_dayahead_prob = zeros(length(offer))
...
for i in 1:length(dt_17)
	if idx_match[i] != 0
		...
		λ_down = df_market17.Down_10[i]
		λ_up = df_market17.Up_10[i]
		revenue_dayahead_prob[i] = offer[i]/1000 * λ_s
		if production > offer[i]
			revenue_balancing_prob[i] = (production - offer[i])/1000 * λ_down
		elseif production < offer[i]
			revenue_balancing_prob[i] = -(offer[i] - production)/1000 * λ_up
		end
	end
end

prob_tot = sum(revenue_balancing_prob)+sum(revenue_balancing_prob)

γ_prob = prob_tot/revenue_optimal_total*100
